...
namespace aruco_mm {
    ...
    bool GlobalGraphMarkerMapper::process(const arucoMarkerSet &in_detected_markers, int imageIdx,
        ...
        if (forceConnectedComponent && kdtrees.size() != 0) {
            ...
            if (!foundAny) {
                markermapper_debug_msg(
                        "Not added because it is not connected to an already added marker. No connection with the group",
                        5);
                return false;
    ...
    void GlobalGraphMarkerMapper::optimize_impl() {
        ...
        if (debug::Debug::getLevel() >= 11)
            read_from_debug_file = debug_read("debug_global_graph.bin", poseGraph, cost_graph, markers_id_pos,
                                              markers_pos_id);
        ...
        _markerSet = calculateMarkerLocationsFromExpansionGraph(falgo, poseGraph, origin_id, markers_id_pos,
                                                                markers_pos_id);
...
//then, we select as best interframe pose as the one that minimizes the reprojection error in all the frames where the two frames are seen
    void GlobalGraphMarkerMapper::compute_pairwise_information(FrameSet &fs, const aruco::CameraParameters &cam_params,
                                                               StgMatrix<se3> &pose_graph,
                                                               graph::Graph<int> &cost_graph,
                                                               std::map <uint32_t, uint32_t> &markers_id_pos) {
        ...
        struct pose_frame {
            pose_frame(const cv::Mat &p, uint32_t f) : pose(p), frame(f) {}
...
#pragma omp parallel for
        for (int fidx = 0; fidx < int(fs.size()); fidx++) {
            ...
            for (size_t m = 0; m < frame.markers.size(); m++) {
                ...
                auto poses = IPPE::solvePnP_(getMarkerPoints(_defaultMarkerSize), marker, cam_params.CameraMatrix,
                                             cam_params.Distorsion);
...
#pragma omp critical
        ...
        auto undistort = [](const vector <cv::Point2f> &in, vector <cv::Point2f> &undp_points,
                            const cv::Mat &CameraMatrix, const cv::Mat &Distorsion) {
...
//fast version of the reproj error. Is not really the reprj error, but is proportional. Input 2d points are already undistorted
        auto get_rpj_err = [](const vector <cv::Point3f> &p3d, const vector <cv::Point2f> &pd_und, const cv::Mat &T,
...
#pragma omp parallel for
        for (int i = 0; i < int(markers_id_pos.size()); i++) {
            auto mi_id = aux_cost_graph(
                    i) = markers_pos_id[i];//do not forget setting the id in the cost_graph!!!!! It will be used later
            ...
            for (size_t j = i + 1; j < markers_id_pos.size(); j++) {
                if (graph(i, j).size() >=
                    size_t(min_num_edges_between_nodes)) {//require at least min_num_edges_between_nodes views to consider the link
                    ...
                    for (auto pose_frame:graph(i, j)) {
                        ...
                        for (auto pf_t:graph(i, j)) {
                            ...
                            if (frame_undpoints[pf_t.frame].size() ==
                                0) //if the 2d points are not yet undistorted, do it now
                                undistort(fs[pf_t.frame].markers.get(mi_id), frame_undpoints[pf_t.frame],
                                          cam_params.CameraMatrix, cam_params.Distorsion);
...
//                        cin.ignore();
                            err += get_rpj_err(marker_points, frame_undpoints[pf_t.frame], c2m,
...
//                        err+=getReprjError(marker_points,fs[pf_t.frame].markers.get(mi_id),cam_params.CameraMatrix,cam_params.Distorsion,c2m);
                    ...
                    markermapper_debug_msg("Best conn " << i << " " << j << "  " << minErr / double(graph(i, j).size()),
                                           8);
    ...
    FrameSet
    GlobalGraphMarkerMapper::getOptimizingFrameSet(FrameSet &fset, MarkerSet &mset, aruco::CameraParameters cam_params,
                                                   double maxRerpjErr, int minNMarkers) {
        FrameSet c_frameSet;
...
#pragma omp parallel for
        for (int i = 0; i < int(fset.size()); i++) {
            ...
            if (int(used_markers.size()) >= minNMarkers) {
                ...
                for (auto marker:used_markers) {
                    ...
                    objpoints.insert(objpoints.end(), p3d.begin(),
                                     p3d.end());//add to vector to compute reprojections later
    ...
    MarkerSet
    GlobalGraphMarkerMapper::calculateMarkerLocationsFromExpansionGraph(graph::Floyd &falgo, StgMatrix<se3> &poseGraph,
                                                                        uint32_t origin_id,
                                                                        std::map <uint32_t, uint32_t> &markers_id_pos,
                                                                        std::map <uint32_t, uint32_t> &markers_pos_id) {
        (void) markers_pos_id;
        int largest_path = -1;
        MarkerSet mset;
    ...
    void GlobalGraphMarkerMapper::debug_save(string fp, StgMatrix<se3> &poseGraph, graph::Graph<int> &cost_graph,
                                             std::map <uint32_t, uint32_t> &markers_id_pos,
                                             std::map <uint32_t, uint32_t> &markers_pos_id) {
    ...
    bool GlobalGraphMarkerMapper::debug_read(string fp, StgMatrix<se3> &poseGraph, graph::Graph<int> &cost_graph,
                                             std::map <uint32_t, uint32_t> &markers_id_pos,
                                             std::map <uint32_t, uint32_t> &markers_pos_id) {
        ifstream file(fp);
...
//extracts the nodes from a graph and creates new graphs
    std::pair <graph::Graph<int>, StgMatrix<se3>>
    GlobalGraphMarkerMapper::extractNodes(graph::Graph<int> &cost_graph, StgMatrix<se3> &poseGraph,
