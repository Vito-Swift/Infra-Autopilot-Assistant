...
/**
 */
    uint32_t PoseGraphOptimizer::optimize(StgMatrix<Pose_Error> &pose_graph_io, bool removeOutlierLinks,
                                          const std::vector<std::string> &nodesInfo) {
...
//    }
        ...
        if (v_gg.size() > 1) {
            ...
            throw std::invalid_argument("there are no component detected in the frame!");
        }
        ...
        if (removeOutlierLinks) {
            ...
            if (nremoved != 0) {
                ...
                falgo.process(best_exp_graph);
                if (getGraphComponents(cost_graph, poseGraph, falgo).size() != 1)
                    throw std::runtime_error("It should not have happened");
            }
        }
...
//    }
        ...
        //cout << "PosegraphCreation="
        //     << double(std::chrono::duration_cast<std::chrono::nanoseconds>(time2 - time1).count()) / 1e9 << "s"
        //     << endl;
...
//finds the different components of a graph
    vector<std::pair<graph::Graph<int>, StgMatrix<se3> > >
    PoseGraphOptimizer::getGraphComponents(graph::Graph<int> &cost_graph, StgMatrix<se3> &poseGraph,
                                           const graph::Floyd &floyd) {

        //extracts the nodes from a graph and creates new graphs
        auto extractNodes = [](graph::Graph<int> &cost_graph, StgMatrix<se3> &poseGraph,
        ...
        std::sort(components.begin(), components.end(), [](const std::pair<graph::Graph<int>, StgMatrix<se3> > &a,
                                                           const std::pair<graph::Graph<int>, StgMatrix<se3> > &b) {
...
//AQUI!!!
    double
    PoseGraphOptimizer::get_expansiongraph_error(graph::Floyd &falgo, graph::Graph<int> &cost_graph, int start_node) {
    ...
    graph::Graph<int>
    PoseGraphOptimizer::get_expansion_graph(graph::Graph<int> &cost_graph, graph::Floyd &falgo, int start_node) {
    ...
    int PoseGraphOptimizer::removeOutlierLinksFromCostGraph(graph::Graph<int> &exp_graph, graph::Graph<int> &cost_graph,
                                                            double ndevs) {
        ...
        for (auto r:toRemove) {
            ...
            if (getNofComponents(cost_graph) > 1) {
                //cout << "attempt to remove link " << r.i << " " << r.j
                //     << " disconnects the graph into ore than one components. Not removed. But there might an oulier connection"
                //     << endl;
    ...
    PoseGraphOptimizer::basic_cycles(graph::Graph<int> &cost_graph, graph::Graph<int> &expansion_graph) {
    ...
    StgMatrix<se3>
    PoseGraphOptimizer::rotational_translational_error_minimization(const vector<vector<uint32_t> > &basic_cycles,
                                                                    StgMatrix<se3> &_mean_graph,
                                                                    graph::Graph<int> &cost_graph) {
    ...
    StgMatrix<se3> PoseGraphOptimizer::rotational_error_minimization(const vector<vector<uint32_t> > &basic_cycles,
                                                                     StgMatrix<se3> &initial_posegraph,
                                                                     const vector<vector<double> > &basic_cycles_weight) {
                ...
                for (auto cycle: basic_cycles) {
                    for (size_t s = 0; s < cycle.size(); s++) {
                        ...
                        for (size_t i = 0; i < cycle.size(); i++) {
                            assert (!isnan(_mean_graph(cycle[(s + i) % cycle.size()],
                                                       cycle[(s + i + 1) % cycle.size()]).getRotation3x3().at<float>(0,
                                                                                                                     0)));
                            r = _mean_graph(cycle[(s + i) % cycle.size()],
                                            cycle[(s + i + 1) % cycle.size()]).getRotation3x3() * r;
                        }
        ...
        auto avrg_Mat = [](const vector<cv::Mat> &vec) {
            cv::Mat res = cv::Mat::zeros(vec[0].size(), vec[0].type());
            for (auto v:vec) res += v;
            res *= 1. / double(vec.size());
            return res;
        };
        ...
        cv::Mat r = cv::Mat::eye(3, 3, CV_32F), aux1(3, 3, CV_32F), aux2(3, 3, CV_32F), rvec(1, 3, CV_32F), sol(3, 3,
                                                                                                                CV_32F);
        do {
            ...
            for (size_t cc = 0; cc < basic_cycles.size(); cc++) {
                ...
                for (size_t s = 0; s < cycle.size(); s++) {
                    ...
                    for (size_t i = 0; i < cycle.size(); i++) {
                        assert (!isnan(curr_posegraph(cycle[(s + i) % cycle.size()],
                                                      cycle[(s + i + 1) % cycle.size()]).getRotation3x3().at<float>(0,
                                                                                                                    0)));

                        curr_posegraph(cycle[(s + i) % cycle.size()], cycle[(s + i + 1) % cycle.size()]).getRotation3x3(
                                aux1);
                        m33mult(aux1, r, aux2);
        ...
        for (size_t cc = 0; cc < basic_cycles.size(); cc++) {
            ...
            for (size_t s = 0; s < cycle.size(); s++) {
                ...
                for (int n = 0; n < 2; n++) {
                    ...
                    auto t_decoupled = initial_posegraph(i, j).getRotation3x3() * c2.t() -
                                       best_posegraph(i, j).getRotation3x3() * c2.t() +
                                       best_posegraph(i, j).getTvec().t();
    ...
    double PoseGraphOptimizer::getTranslationalError(const vector<vector<uint32_t> > &basic_cycles,
                                                     StgMatrix<se3> &_mean_graph) {
        //total rotational error
    ...
    StgMatrix<se3> PoseGraphOptimizer::translational_error_minimization(const vector<vector<uint32_t> > &basic_cycles,
                                                                        StgMatrix<se3> &_graph,
                                                                        const vector<vector<double> > &basic_cycles_weights) {
...
//            RT=  rt.convert()*RT;
        ...
        for (size_t i = 0; i < tcorrected_graph.rows(); i++)
            for (size_t j = 0; j < tcorrected_graph.cols(); j++)
                if (tcorrected_graph(i, j).isValid() && !tcorrected_graph(j, i).isValid())
                    resulting_graph(j, i) = resulting_graph(i, j).inverse();
