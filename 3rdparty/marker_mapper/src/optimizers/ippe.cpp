...
std::vector <cv::Mat>
IPPE::solvePnP(const std::vector <cv::Point3f> &objPoints, const std::vector <cv::Point2f> &imgPoints,
...
std::vector <std::pair<cv::Mat, double>>
IPPE::solvePnP_(const std::vector <cv::Point3f> &objPoints, const std::vector <cv::Point2f> &imgPoints,
                cv::InputArray cameraMatrix, cv::InputArray distCoeffs) {
    cv::Mat Rvec, Tvec;
    float markerLength = cv::norm(objPoints[1] - objPoints[0]);
    float reprojErr1, reprojErr2;
    ...
    IPPE::solvePoseOfCentredSquare(markerLength, imgPoints, cameraMatrix, distCoeffs, Rvec, Tvec, reprojErr1, Rvec2,
                                   Tvec2, reprojErr2);
    return {std::make_pair(aruco_mm::getRTMatrix(Rvec, Tvec, CV_32F), reprojErr1),
            std::make_pair(aruco_mm::getRTMatrix(Rvec2, Tvec2, CV_32F), reprojErr2)};

}

void IPPE::solvePoseOfCentredSquare(float squareLength, InputArray imagePoints, InputArray cameraMatrix,
                                    InputArray distCoeffs,
                                    OutputArray _rvec1, OutputArray _tvec1, float &reprojErr1, OutputArray _rvec2,
                                    OutputArray _tvec2, float &reprojErr2) {
...
float
IPPE::IPPEvalReprojectionError(InputArray _R, InputArray _t, InputArray _objectPoints, InputArray _undistortedPoints) {
    cv::Mat modelPoints = _objectPoints.getMat();
    cv::Mat imgPoints = _undistortedPoints.getMat();
...
void IPPE::homographyFromSquarePoints(InputArray _targetPts, double halfLength, OutputArray H_) {
    ...
    //analytic solution:
    double detsInv = -1 / (halfLength *
                           (p1x * p2y - p2x * p1y - p1x * p4y + p2x * p3y - p3x * p2y + p4x * p1y + p3x * p4y -
                            p4x * p3y));

    H.at<double>(0, 0) = detsInv *
                         (p1x * p3x * p2y - p2x * p3x * p1y - p1x * p4x * p2y + p2x * p4x * p1y - p1x * p3x * p4y +
                          p1x * p4x * p3y + p2x * p3x * p4y - p2x * p4x * p3y);
    H.at<double>(0, 1) = detsInv *
                         (p1x * p2x * p3y - p1x * p3x * p2y - p1x * p2x * p4y + p2x * p4x * p1y + p1x * p3x * p4y -
    ...
    H.at<double>(1, 0) = detsInv *
                         (p1x * p2y * p3y - p2x * p1y * p3y - p1x * p2y * p4y + p2x * p1y * p4y - p3x * p1y * p4y +
                          p4x * p1y * p3y + p3x * p2y * p4y - p4x * p2y * p3y);
    H.at<double>(1, 1) = detsInv *
                         (p2x * p1y * p3y - p3x * p1y * p2y - p1x * p2y * p4y + p4x * p1y * p2y + p1x * p3y * p4y -
