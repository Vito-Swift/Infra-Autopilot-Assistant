...
namespace aruco_mm {
    ...
    double FullSceneOptimizer::optimize(
            MarkerSet &MarkerSet,
            FrameSet &FrameSet, const std::set<uint32_t> &fixedMarkers, aruco::CameraParameters &camp,
            bool fix_cameraparams) {
    ...
    SparseLevMarq<double>::eVector
    FullSceneOptimizer::toVector(MarkerSet &MarkerSet, FrameSet &FrameSet, aruco::CameraParameters &cam_params) {
    ...
    void FullSceneOptimizer::fromVector(SparseLevMarq<double>::eVector &sol, MarkerSet &MarkerSet, FrameSet &FrameSet,
    ...
    std::vector<float>
    FullSceneOptimizer::getSolution(MarkerSet &MarkerSet, FrameSet &FrameSet, aruco::CameraParameters &cam_params) {
    ...
    double FullSceneOptimizer::optimize(MarkerSet &MarkerSet, FrameSet &FrameSet, aruco::CameraParameters &cam_params,
                                        Params &params) {
...
// cout<<"MIN STEP ERROR="<<minStepError<<endl;
        ...
        solver.solve(sol, std::bind(&FullSceneOptimizer::error_optimization, this, std::placeholders::_1,
                                    std::placeholders::_2),
                     std::bind(&FullSceneOptimizer::error_jacobian, this, std::placeholders::_1,
                               std::placeholders::_2));
        error_optimization(sol, error); //cout<<"ERRXX FINAL="<<error.cwiseProduct(error).sum()<<endl;
        stats(error);
...
//    }
    ...
    void FullSceneOptimizer::error_optimization(const SparseLevMarq<double>::eVector &sol,
                                                SparseLevMarq<double>::eVector &error) {
        ...
        std::unordered_map<uint32_t, vector < cv::Point3f> > markers_points3d;
        {
            ...
            for (size_t i = 0; i < sba_markerSet.size(); i++, marker++) {
                ...
                markers_points3d.insert(std::make_pair(marker->second.id, getMarkerPoints(marker->second.markerSize,
                                                                                          aruco_mm::getRTMatrix(r,
                                                                                                                t))));
            }
        }
        ...
        for (auto frame: sba_frameSet)
            errorvectorsize += frame.second.markers.size() *
                               8;//each markers provides 4 points, with two measures each(x,y)
...
//            error(currErrRow+1)= sba_ba_params.error_func(ey);
                ...
                if (sba_ba_params.fixedFrames.find(frame.frame_idx) ==
                    sba_ba_params.fixedFrames.end()) {//not a fixed frame
                    //r and t
                    for (int rti = 0; rti < 6; rti++) {
                        jac_omp[omp_tid].push_back(
                                Eigen::Triplet<double>(currErrRow, frameOffset + rti, jacobian.at<double>(p * 2, rti)));
                        jac_omp[omp_tid].push_back(Eigen::Triplet<double>(currErrRow + 1, frameOffset + rti,
                                                                          jacobian.at<double>(p * 2 + 1, rti)));
                    }
                }
                if (!sba_ba_params.fix_camera_params) {
                    //camera paramrs
                    for (int rti = 0; rti < 9; rti++) {
                        jac_omp[omp_tid].push_back(Eigen::Triplet<double>(currErrRow, cameraParamsOffset + rti,
                                                                          jacobian.at<double>(p * 2, rti + 6)));
                        jac_omp[omp_tid].push_back(Eigen::Triplet<double>(currErrRow + 1, cameraParamsOffset + rti,
                                                                          jacobian.at<double>(p * 2 + 1, rti + 6)));
                    }
                }
            }
        }
    ...
    void
    FullSceneOptimizer::error_jacobian(const SparseLevMarq<double>::eVector &sol, Eigen::SparseMatrix<double> &sp) {
        ...
        struct marker_frame_projection {
            marker_frame_projection(int f, int s, cv::Mat R, cv::Mat T) {
                frame_idx = f;
                start_row = s;
                frame_r = R;
                frame_t = T;
...
#pragma omp parallel for
        for (int markerIdx = 0; markerIdx < int(sba_markerSet.size()); markerIdx++) {
            ...
            if (sba_ba_params.fixedMarkers.find(marker.id) ==
                sba_ba_params.fixedMarkers.end()) {//The fixd markers derivatives are set of 0 der to avoid moving them
                ...
                for (auto frame:sba_frameSet) {//check across frames
                    for (auto m:frame.second.markers) {//see if the marker is in the frame
                        if (m.id == marker.id) {
                            ...
                            mfv.push_back(
                                    marker_frame_projection(frame.second.frame_idx, proj_off, r, t));//add it to list
                        }
                ...
                for (int rt = 0; rt < 6; rt++) {
                    ...
                    for (auto mfp: mfv) {
                        ...
                        for (int p = 0; p < 4; p++) {
                            ...
                            jac_omp[omp_thread].push_back(
                                    Eigen::Triplet<double>(errorRow++, markerIdx * 6 + rt, derv_errx));
                            double derv_erry = (img_points_p_n[p].y - img_points_p_n[p + 4].y) / (2. * der_epsilon);
                            jac_omp[omp_thread].push_back(
                                    Eigen::Triplet<double>(errorRow++, markerIdx * 6 + rt, derv_erry));
